...
closed-loop floor, and drops stay under ~0.1%. It is bounded by throughput and
p50, so read the p99 column with it rather than assuming the tail is tight
everywhere inside the zone — braft's is not at the top of its own.
...
braft's zone runs right up to its knee: at 70k throughput is still exact and
drops are nil, but p99 has already reached 2.7x its 969 µs floor while p50 has
moved only 14%. Whether that top end is usable is a per-workload call — 70k with
a 2.6 ms tail is fine for a workload that reads p50 and pathological for one with
a 1 ms deadline. Stopping at 55k keeps p99 at 1070 µs for 21% less throughput.
...
29% above the rate at which its tail had already broken. On its chart the band edge
is the first knee and the dashed line is the second.
